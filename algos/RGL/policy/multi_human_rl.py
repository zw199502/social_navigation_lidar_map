...
import logging
import numpy as np
from utils.action import ActionRot, ActionXY
...
class MultiHumanRL(CADRL):
    ...
    def predict(self, state):
        ...
        if self.phase == 'train' and probability < self.epsilon:
            ...
            max_value = float('-inf')
            max_action = None
            goal_distance_last = hypot(state.self_state.gy - state.self_state.py, state.self_state.gx - state.self_state.px)
            for action in self.action_space:
                next_robot_state = self.propagate(state.self_state, action)
                next_human_states = [self.propagate(human_state, ActionXY(human_state.vx, human_state.vy))
                                        for human_state in state.human_states]
                reward = self.compute_reward(next_robot_state, next_human_states, goal_distance_last)
        ...
        if self.kinematics == 'holonomic':
            action_array = np.array([max_action.vx, max_action.vy], dtype=np.float32)
        else:
            action_array = np.array([max_action.v, max_action.r], dtype=np.float32)

        return action_array

    def compute_reward(self, nav, humans, goal_distance_last):
        # to do: parameters should be consitent with those as defined for environment
        success_reward = 0.5
        collision_penalty = -0.5
        discomfort_dist = 0.5
        discomfort_penalty_factor = 0.4
        goal_distance_factor = 1.4
        # if reaching goal
        goal_dist = hypot(nav.px - nav.gx, nav.py - nav.gy)
        reaching_goal = goal_dist < (nav.radius + 0.1)

        # collision detection between the robot and humans
        collision = False
        dmin = 9999.9
        for i, human in enumerate(humans):
            dis_human = hypot(human.px - nav.px, human.py - nav.py)
            dmin = min(dmin, dis_human - nav.radius - human.radius)
            if dmin <= nav.radius:
                collision = True
                break
   
        reward = 0
        if ((dmin - nav.radius) < discomfort_dist):
            dis_obstacle_reward = (dmin - nav.radius - discomfort_dist) * discomfort_penalty_factor
        else:
            dis_obstacle_reward = 0.0
            
        dis_goal_reward = goal_distance_factor * (goal_distance_last - goal_dist)
    ...
    def transform(self, state):
        ...
        rotated_state_tensor = rotated_state_tensor.to(torch.float32)

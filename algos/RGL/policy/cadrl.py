...
import numpy as np
import itertools
from algos.RGL.policy.policy import Policy
...
class CADRL(Policy):
    ...
    def build_action_space(self, v_pref):
        ...
        speeds = np.linspace(0.0, v_pref, self.speed_samples)
        if holonomic:
    ...
    def propagate(self, state, action):
        if isinstance(state, ObservableState):
            ...
            if self.kinematics == 'holonomic':
                ...
                if next_theta > np.pi:
                    next_theta -=  2.0 * np.pi
                elif next_theta < -np.pi:
                    next_theta += 2.0 * np.pi
                # LIP model
                last_v = np.hypot(state.vx, state.vy)
                pf_x = (last_v * self.cosh_wt - action.v) / (self.w * self.sinh_wt)
                x_n =  pf_x - pf_x * self.cosh_wt + last_v * self.sinh_wt / self.w
                next_px = state.px + x_n * np.cos(next_theta)
                next_py = state.py + x_n * np.sin(next_theta)

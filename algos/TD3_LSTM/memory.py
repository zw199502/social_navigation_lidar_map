...
class ReplayBuffer(object):
    def __init__(
        self, lidar_state_dim, position_state_dim, digit_dim, action_dim, hidden_size, max_size, device
    ):
        ...
        self.position_robot_state = np.zeros((self.max_size, position_state_dim + digit_dim), dtype=np.float32)
        self.action = np.zeros((self.max_size, action_dim), dtype=np.float32)
        self.next_lidar_state = np.zeros((self.max_size, lidar_state_dim), dtype=np.float32)
        self.next_position_state = np.zeros((self.max_size, position_state_dim + digit_dim), dtype=np.float32)
